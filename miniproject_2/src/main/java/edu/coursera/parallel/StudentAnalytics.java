...
//        System.out.println("Time to averageAgeOfEnrolledStudentsParallelStream: " + (endTime - startTime) + " with result = " + result);
        return result;
    }
...
//        throw new UnsupportedOperationException();
        ...
        nameCount = Arrays.stream(studentArray)
                .parallel()
                .filter(s -> !s.checkIsCurrent())
        ...
        result = nameCount.entrySet()
                .parallelStream()
                .max(Map.Entry.comparingByValue(Comparator.comparingLong(Long::longValue)))
                .get().getKey();
